...
const planetName = document.querySelectorAll(".planetName");
...
function runCodeOnce() {
    if (!hasRun) {
        planetName[0].classList = "planetName white";
...
function choosePlanet(i) {
    if (planetName[i].classList.contains("white")) {
        ...
        planetName[i].classList = "planetName white";
        number = i
    ...
    for (let k = 0; k < planetName.length; k++) {
        if (
            ...
            planetName[k].classList = "planetName";
        }
    }
}
// choose planets
const line = document.querySelectorAll(".hoverLine");
